...
function validate( opts, options ) {
	...
	if ( hasOwnProp( options, 'submode' ) ) {
		if ( !isArrayLikeObject( options.submode ) ) {
			return new TypeError( format( '0jp5b', 'submode', options.submode ) );
		}
		if ( options.submode.length === 0 ) {
			return new TypeError( format( '0jp5b', 'submode', join( options.submode, ',' ) ) );
		}
		tmp = [];
		for ( i = 0; i < options.submode.length; i++ ) {
			if ( !isIndexMode( options.submode[ i ] ) ) {
				return new TypeError( format( '0jp5c', options.submode[ i ] ) );
			}
